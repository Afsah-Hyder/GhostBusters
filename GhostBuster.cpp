...
void initialize(){
    ...
    ghostRow= rand()% rows; 
    ghostCol= rand() % cols;
    cout<<"Ghost row: "<<ghostRow<<". Ghost Column "<<ghostCol<<endl;
    for(int i=0;i<(rows*cols) ;i++){       //single loop since the grid is a one-dimensional array
        grid[i]='L';                      //entire grid initiallized with locks
    }
}
...
void huntGhost(int x, int y){
    ...
    int user_col; int user_row;
    user_col=x/(width/cols);       // x-coordinate of the block
    user_row=y/(height/rows);      // y-coordinate of the block
    int distance;
    distance=sqrt(pow(user_row-ghostRow,2)+pow(user_col-ghostCol,2));   // distance formula: between user location and the ghost
    if (ended==false){                          // conditions only checked if user has not right cicked yet
    if ((distance>=0)&&(distance<=1)){
        grid[(user_row*cols)+user_col]='S';
    }
    else if((distance>=2)&&(distance<=3)){
        grid[(user_row*cols)+user_col]='T';
    }
    else if(distance==4){
        grid[(user_row*cols)+user_col]='B';
    }
    else{
        char words[]={'S','T','B'};                      //the three animals char are stored in this words array
        grid[(user_row*cols)+user_col]=words[rand()%2];      // one of the three animals is chosen at random if the user is more than 4 blocks away
    }
    }
}
...
void bustGhost(int x, int y){
    ...
    int final_row; int final_col;
    final_row=y/(height/rows);      //y-coordinate of the block
    final_col=x/(width/cols);       //x-coordinate of the block
    if((final_row==ghostRow)&&(final_col==ghostCol)){      //if user has right-clicked on the location of the ghost
        grid[(final_row*cols)+final_col]='G';              //the ghost appears if right clicked on the correct location
        ended=true;
    }
    else{
        grid[(final_row*cols)+final_col]='F';             //'fail' appears if right clicked on the wrong location
        ended=true;
    }  
}
...
void quitGhostBuster(){
    ...
    if (ended==true){
        delete [] grid;           //deletes grid memory location
        grid=NULL;
    }
}